...
if sudo systemctl is-active --quiet $SERVICE_NAME; then
    ...
    warning "⚠️  Сервис не запустился, проверяем причину..."
    ...
    log "🔍 Проверяем бинарный файл:"
    ls -la $PROJECT_DIR/server/websocket-server
    
    # Проверяем права доступа
    log "🔐 Проверяем права доступа:"
    sudo -u www-data $PROJECT_DIR/server/websocket-server --help 2>&1 || warning "Бинарный файл не запускается"
    
    error "❌ Ошибка при запуске сервиса $SERVICE_NAME. Проверьте логи выше."
fi
